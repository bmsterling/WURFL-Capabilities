...
class WURFL_FileUtils {
	...
	/**
	 * Create a directory structure if directory doesn't already exists
	 * @param string $path
	 * @param int $mode
	 * @param boolean $recursive
	 */
	public static function safe_mkdir($path, $mode=0755, $recurse=true) {
		if( file_exists($path) && is_dir($path) ) {
			return true;
		} else {
			return @mkdir($path, $mode, $recurse);
		}
	}
